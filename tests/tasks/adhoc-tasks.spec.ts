...
export default async function adhocTask(page: Page, testData: TestData) {
    ...
    try {
        ...
        console.log('Submitting adhoc task: clicking "Indienen"...');
        await page.getByRole('button', { name: 'Indienen' }).click();
        console.log(`Adhoc task completed successfully with option "${option}".`);

        // 🔁 Follow-up tasks AFTER adhoc completion
        // After buiten behandeling stellen
        if (option === 'A') {
            console.log(
